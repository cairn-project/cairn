...
> **Name note.** "Cairn" is a working placeholder; the final, mission-neutral
> public name is not yet ratified. Where a final repo URL or public name would
> appear below, you will see a clearly-marked `[placeholder]`.
...
- **Understand the safety frames.** Every significant design choice in this
  project is checked against five non-negotiable frames (works / doesn't-target-
  good-people / legal / court-grade-auditable / human-verified). See the
  project plan and `BUILD-PLAN.md`. A change that weakens any frame will be
  declined regardless of how clean the code is.
...
1. **Fork** the repository (once the public repo exists — `[placeholder]`).
...
- **General questions / design discussion:** open a
  [discussion or issue](.github/ISSUE_TEMPLATE/) on the public repo
  (`[placeholder]` until the repo is stood up).
