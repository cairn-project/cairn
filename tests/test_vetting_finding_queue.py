...
def test_flag_finding_lands_pending_content_addressed_and_logged(tmp_path):
    ...
    finding = queue.flag(packet_hash=_PACKET, automated_verdict=_VERDICT, flagged_by="node-1")
...
def test_routable_only_after_recorded_routable_verdict(tmp_path):
    ...
    item = queue.record_verdict(
        finding.finding_hash, routable=True, reason="human confirms", reviewer_id="vetter-2"
    )
...
def test_reject_without_reason_refused_no_silent_rejection(tmp_path):
    ...
    with pytest.raises(VettingError):
        queue.record_verdict(
            finding.finding_hash, routable=False, reason="", reviewer_id="vetter-2"
        )
...
def test_double_verdict_refused(tmp_path):
    ...
    queue = FindingVetQueue(ledger)
    finding = queue.flag(packet_hash=_PACKET, automated_verdict=_VERDICT, flagged_by="node-1")
    queue.record_verdict(finding.finding_hash, routable=True, reason="ok", reviewer_id="vetter-2")
    with pytest.raises(VettingError):
        queue.record_verdict(
            finding.finding_hash, routable=False, reason="changed mind", reviewer_id="vetter-2"
        )
